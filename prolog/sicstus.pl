/*  Part of SWI-Prolog
    ...
    Additions: Add sublist
    Author: Olivier Sallou <olivier.sallou@irisa.fr>
    Copyright : 2011, IRISA
*/
...
%	    prolog_flag/2,		% +Flag, -Value
	    ...
	    version/1,			% +Message
            ...
            sublist/5,                  % +InList, -OutList, +Before, -Length, +After
...
sublist([X|Y],FinalList,Before,Length,After):- length([X|Y],Z),((var(Length),Length is (Z - After - Before));(var(After),After is (Z - Before - Length))),sublist([X|Y],[],Before,Length,0,0,FinalList).

sublist([],TmpList,Before,Length,CountPos,CountLength,FinalList):-FinalList = TmpList.
sublist([X|Y],TmpList,Before,Length,CountPos,CountLength,FinalList):- 
(CountPos<Before,NewCountPos is CountPos +1,sublist(Y,TmpList,Before,Length,NewCountPos,CountLength,FinalList))
;
(CountPos=Before,
(
(CountLength<Length,(append(TmpList,[X],NewList),NewCountLength is CountLength + 1,sublist(Y,NewList,Before,Length,CountPos,NewCountLength,FinalList)))
;
(CountLength=Length,FinalList = TmpList)
)
).
